...
namespace BellCranel\LuckyWars;
...
use BellCranel\LuckyWars\ResetMap;
...
class LuckyWars extends PluginBase implements Listener {
    ...
    public $prefix = TE::GRAY . "§1-§2" . TE::YELLOW . TE::BOLD . "§fLucky" . TE::GREEN . "§6Wars" . TE::RESET . TE::GRAY . "§1-§2";
	public $mode = 0;
	public $arenas = array();
	public $currentLevel = "";
        public $op = array();
	...
	{
		  $this->getLogger()->info(TE::DARK_AQUA . "LuckyWars");
        ...
        switch($cmd->getName()){
			case "bc1":
				if($player->isOp())
				{
					...
					{
					 $player->sendMessage("§fLuckyWars Commands!");
                                         $player->sendMessage("§e/bc1 lw [world]: Create a lw game!");
                                         $player->sendMessage("§e/lwstart: start the game");
					}
				}
			return true;
                            ...
                            if($player->isOp())
				{
                                if(!empty($args[0]))
					{
                                        ...
                                        $config->save();
                                        $player->sendTitle("§aEmpezando en 10...");
                                        }
                                        }
                                        else
                                        {
					...
					else
					{
						...
						if($config->get($arena . "inicio")==1)
						{
							if($timeToStart>0)
							{
								...
								{
									$pl->sendTitle(TE::YELLOW."Comenzando en ".TE::GREEN . $timeToStart.TE::RESET);
                                                                        if($timeToStart<=5)
                                                                        {
